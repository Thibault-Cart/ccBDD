CREATE OR REPLACE TRIGGER TRG_INSERT_VW_EXA_PROJETS
  INSTEAD OF INSERT ON VW_EXA_PROJETS
  FOR EACH ROW
DECLARE
...
BEGIN
  ...
  IF (V_PROJET_EXISTE < 0) THEN
    ...
    SELECT C.CAT_NO
      ...
      FROM EXA_CATEGORIE C
     WHERE C.CAT_NOM LIKE V_PROJET_CAT_NOM;
    -- insertion projet dans la base
    INSERT INTO EXA_PROJET
    VALUES
      (NULL, :NEW.PROJET, :NEW.DESCRIPTION, :NEW.PRIORITE, V_CAT_NO);
  ...
  DBMS_OUTPUT.PUT_LINE(:NEW.PRIORITE);
END TRG_INSERT_VW_EXA_PROJETS;
/
